...
module CategoryMatcher =
    ...
    // http://www.fssnip.net/bj
    let private levenshtein word1 word2 =
    ...
    let getPotentialCategoryMatches description (categories:Category list) =
        let getOptimalMatchInCategory category =
            category.DescriptionsToMatch
            |> List.map (fun categoryDescription -> (categoryDescription.ToLowerInvariant(), fst (levenshtein (categoryDescription.ToLowerInvariant()) description))) 
            |> List.minBy (fun (categoryDescription, distance) -> distance)
            |> (fun (categoryDescription, distance) -> (category, categoryDescription, distance))

        categories
        |> List.map getOptimalMatchInCategory
